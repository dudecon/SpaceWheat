...
qii_token "S.QIIGATE" "script"
qii_token "G3G.Q" "build_bell"
qii_token "QC.MI" "mutual_info"
...
set +e
{
  ...
  echo "Command: godot --headless --path . --script Tests/qii_gate_entangle_probe.gd"
  echo
  godot --headless --path . --script Tests/qii_gate_entangle_probe.gd
} 2>&1 | tee "$log_file"
status=${PIPESTATUS[0]}
set -e
