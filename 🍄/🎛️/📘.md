...
This rig lets "Codex‑mini" agents play **one turn at a time** against a **single live game session** (BootManager → PlayerShell → FarmInstrument). You submit JSON turns, the game executes them, and it appends a JSON result line.
...
2. **Submit turns (one at a time)**:
   ...
   - `🍄/🎛️/✍️.sh '{"turn":4,"action":"active_quests"}'`
   - `🍄/🎛️/✍️.sh '{"turn":5,"action":"grid_snapshot"}'`
