...
qii_token "S.QIIVSEQ" "script"
qii_token "G4.Q" "inject village"
qii_token "GRID.CHK" "grid after village"
qii_token "G4.Q" "inject forest"
qii_token "GRID.CHK" "grid after forest"

set +e
{
  echo "Experiment: ${emoji}"
  echo "Started: $(date -Is)"
  echo "Command: godot --headless --path . --script Tests/qii_vocab_sequence.gd"
  echo
  godot --headless --path . --script Tests/qii_vocab_sequence.gd
} 2>&1 | tee "$log_file"
status=${PIPESTATUS[0]}
set -e
